...
class RsInferenceContext(
    ...
    private val constUnificationTable: UnificationTable<CtInfer.CtVar, Const> = UnificationTable()
    ...
    private fun combineIntOrFloatVar(ty1: TyInfer, ty2: Ty): RelateResult {
        when (ty1) {
            is TyInfer.IntVar -> when (ty2) {
                ...
                else -> return RelateResult.TypeMismatch(ty1, ty2)
            }
    ...
    fun combineTypesNoVars(ty1: Ty, ty2: Ty): RelateResult =
        when {
            ...
            else -> RelateResult.TypeMismatch(ty1, ty2)
        }
    ...
    private fun combineConstsResolved(const1: Const, const2: Const): RelateResult =
        when {
            const1 is CtInfer.CtVar -> combineConstVar(const1, const2)
            const2 is CtInfer.CtVar -> combineConstVar(const2, const1)
            else -> combineConstsNoVars(const1, const2)
        }

    private fun combineConstVar(const1: CtInfer.CtVar, const2: Const): RelateResult {
        if (const2 is CtInfer.CtVar) {
    ...
    private fun combineConstsNoVars(const1: Const, const2: Const): RelateResult =
        when {
            const1 === const2 -> RelateResult.Ok
            const1 is CtUnknown || const2 is CtUnknown -> RelateResult.Ok
            const1 == const2 -> RelateResult.Ok
            else -> RelateResult.ConstMismatch(const1, const2)
        }
    ...
    fun <T : TypeFoldable<T>> shallowResolve(value: T): T {
        val resolver = ShallowResolver()
        return value.foldWith(resolver)
    }
    ...
    fun <T : TypeFoldable<T>> resolveTypeVarsIfPossible(value: T): T {
        val resolver = OpportunisticVarResolver()
        return value.foldWith(resolver)
    }
    ...
    fun constVarForParam(const: CtConstParameter): Const = CtInfer.CtVar(const)
...
sealed class RelateResult {
    ...
    class TypeMismatch(val ty1: Ty, val ty2: Ty) : RelateResult()
    class ConstMismatch(val const1: Const, val const2: Const) : RelateResult()

    val isOk: Boolean get() = this == Ok
}

inline fun RelateResult.and(rhs: () -> RelateResult): RelateResult = if (isOk) rhs() else this
...
fun RelateResult.into(): CoerceResult = when (this) {
    ...
    is RelateResult.TypeMismatch -> CoerceResult.TypeMismatch(ty1, ty2)
    is RelateResult.ConstMismatch -> CoerceResult.ConstMismatch(const1, const2)
}
